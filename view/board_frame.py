...
class BoardFrame(ctk.CTkFrame):
    ...
    def __init__(self, board: MinesweeperModel, master, **kwargs) -> None:
        ...
        self.buttons = [
            [None for _ in range(self._board.length)]
    ...
    def activate_button(
        self, button: ctk.CTkButton, row: int, col: int
    ) -> None:
        """
        ...
        button.configure(
            command=lambda r=row, c=col: self.reveal_square_value(r, c)
        )
    ...
    def disable_button(
        self, row: int, col: int, clear_text: bool = False
    ) -> None:
        """
    ...
    def reveal_adjacent_blanks(self, coords: list[list[int]]) -> None:
        ...
        for cmod_x, cmod_y in self._coord_mods:
            self.reveal_adjacent_blanks(
                [coords[0] + cmod_x, coords[1] + cmod_y]
            )
        ...
        match self._board.get_value_at([row, col]):
            ...
            case _:
                self.buttons[row][col].configure(
                    text=f"{self._board.get_value_at([row, col])}"
                )
